...
const injectStyles = () => {
  ...
  style.textContent = `
    ...
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    .field-wrap:nth-child(1){animation:fieldIn 0.4s 0.1s ease both;}
    .field-wrap:nth-child(2){animation:fieldIn 0.4s 0.2s ease both;}
    ...
    .forgot-row { text-align:right;margin-top:-4px;animation:fieldIn 0.4s 0.35s ease both; }
    .forgot-row a { font-size:12.5px;color:#FF6600;text-decoration:none;font-weight:700; }
    ...
    .auth-switch { text-align:center;font-size:13.5px;color:#aaa;animation:fieldIn 0.4s 0.65s ease both; }
    ...
    @media(max-width:500px) {
      .auth-card{width:95vw;} .auth-body{padding:22px 20px 28px;}
...
export default function Login() {
  ...
  const { login }       = useAuth(); // ✅ REAL AuthContext
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true);
    try {
      // ✅ FIX: pass ONE object — matches AuthContext: login(data)
      // which calls: API.post("/auth/login", data)
      const user = await login({ email, password });

      // user.role comes from your real MongoDB user document
